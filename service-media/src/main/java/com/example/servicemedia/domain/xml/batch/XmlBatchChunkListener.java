...
public class XmlBatchChunkListener implements ChunkListener {
    ...
    public void beforeChunk(@NonNull ChunkContext context) {
        ...
        definition.setJobExecutionId(context.getStepContext().getStepExecution().getJobExecutionId());
    ...
    private XmlDefinition getDefinition(ChunkContext context) {
        final String definitionId = context.getStepContext().getStepExecution().getJobExecution().getJobParameters().getString(BATCH_DEFINITION_ID);
